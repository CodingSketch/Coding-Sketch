...
#include<bits/stdc++.h>
using namespace std;
class BIT_CM_ver2 :public Sketch{
    public:
    class Layer{
        char *counter;
        char *bitmap;
        int len,bit,mx,d,hashseed,level;
        Layer*upper;
        ...
        void cover(int pos,int c){
            if(c>mx||c<0||pos>len){
                fprintf(stderr,"%d cover error %d %d\n",level,pos,c);
                if (c>mx) 
                {
                    fprintf(stderr,"reason 1, c=%d, mx=%d\n",c,mx);
                }
                if (c<0) 
                {
                    fprintf(stderr,"reason 2, c=%d\n",c);
                }
                if (pos>len) 
                {
                    fprintf(stderr,"reason 3, pos=%d, len=%d\n",pos,len);
                }
                exit(-1);
            }
            // printf("cover %d %d\n",pos,c);
    ...
    public:
        Layer(int _w=0,int _bit=1,int _d=3,int _hashseed=1000,Layer*up=NULL,int _id=0){
            len=_w/(_bit);
            bit=_bit;
            d=_d;
            hashseed=_hashseed;
    ...
    int *c0;
public:
    BIT_CM_ver2(int _w,int _d,int _hashseed=1000){
        _w*=8;
        w=_w;
        d=_d;
        hashseed=_hashseed;
        ...
        layer[3]=Layer(sz[3]*2,2,3,hashseed*4,&layer[4],3);
        layer[4]=Layer(sz[4]*2,2,3,hashseed*5,&layer[5],4);
        layer[5]=Layer(sz[5]*10,10,3,hashseed*6,NULL,5);
        c0=new int[layer[0].Len()]();
    }
    int layer0_len() {return layer[0].Len();}
    void temp_insert(int pos, int k)
    {
        layer[0].Insert(pos,k);
        c0[pos]+=k;
    }
	...
	int Query(const char* str){
        if(unzip==0){
            ...
            for(int i=0;i<len;i++)
            {
                ...
                {
                    sum++;
                    //printf("\nindex = %d, counter[%d] = %d, c0[%d] = %d\n", i, i, counter[i], i, c0[i]);
                }
            }
