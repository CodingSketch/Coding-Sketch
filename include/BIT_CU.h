...
class BIT_CU :public Sketch{
    ...
    public:
        ...
        int easyquery(int pos){
            int nw=ask(pos);
            if(askbit(pos)){
                int up=INT_MAX;
                for(int index,i=0;i<d;i++){
                    index=((unsigned)MurmurHash32(&pos,4, hashseed+i))%upper->Len();
                    up=min(up,upper->easyquery(index));
                }
                nw+=up*(mx+1);
            }
            return nw;
    ...
    int *c0;
public:
    BIT_CU(int _w,int _d,int _hashseed=1000){
        _w*=8;
        w=_w;
        d=_d;
        hashseed=_hashseed;
	...
	void Insert(const char* str){
		int g=0,index[MAX_HASH_NUM],min_value=INT_MAX;
		for (int i = 0; i < d; i++){
			index[i] = ((unsigned)MurmurHash32(str, KEY_LEN, hashseed + i)) % layer[0].Len();
            min_value=min(min_value,layer[0].easyquery(index[i]));
		}
		for (int i = 0; i < d; i++){
			if (layer[0].easyquery(index[i]) == min_value){
